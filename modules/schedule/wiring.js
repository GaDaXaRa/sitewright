...
export const wiring = {
  ...
  collectionCall: (m) =>
    `scheduleCollection({ labels: ${JSON.stringify(m.labels)}, route: ${sourceString(m.route)}${m.online ? ', place: false' : ''} })`,
  ...
  jsonldNodes: (m) => `...scheduleNodes(splitEvents(schedule, now).upcoming, ${sourceString(m.route)})`,
  ...
  if (scheduleCount.totalDocs === 0) {
    ...
    const examples = [
      { title: ${sourceString('Próxima ' + m.labels.singular.toLowerCase())}, startsAt: day(10, 19), endsAt: day(10, 21) },
    ...
    for (const example of examples) {
      await payload.create({
        collection: 'schedule',
        data: { ...example, venue: 'Lugar por decidir', city: ${sourceString(m.city ?? 'Ciudad')}, free: true },
      })
    }
