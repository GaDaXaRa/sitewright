...
export default function SchedulePage({ items, settings, now, title, route }: ModulePageProps) {
  ...
  // No usa el vacío del armazón: sin nada por delante la página **no** está vacía si tiene
  // archivo, y lo que hay que decir va dentro de «Próximas», no en lugar de todo.
  return (
    <ModuleIndexPage
      settings={settings}
      title={title}
      route={route}
      nodes={scheduleNodes(list, route)}
    >
